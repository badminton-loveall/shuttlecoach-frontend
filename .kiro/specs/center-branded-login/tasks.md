...
- [x] 1. Database migration and slug utility
  ...
  - [x] 1.2 Write property test for slug validation (Property 1)
  ...
  - [x]* 1.3 Write property test for slug generation (Property 2)
...
- [x] 2. Public center info API endpoint
  - [x] 2.1 Create public centers controller
  ...
  - [x] 2.2 Register public center info route
    - Add `GET /api/centers/:slug/info` route with no auth middleware
    - Wire to `getCenterInfo` handler
    - _Requirements: 2.1_

  - [x]* 2.3 Write property test for response field restriction (Property 3)
...
- [x] 3. Checkpoint - Public API complete
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Center-scoped login validation
  - [x] 4.1 Modify auth controller to accept centerSlug
  ...
  - [x]* 4.2 Write property test for center-scoped login validation (Property 4)
...
- [x] 5. Admin center slug update
  - [x] 5.1 Extend admin centers controller to accept slug field
  ...
  - [x]* 5.2 Write property test for slug management role restriction (Property 5)
...
- [x] 7. Frontend - Branded login page
  - [x] 7.1 Add CenterPublicInfo type and API service function
  ...
  - [x] 7.2 Enhance LoginPage to support center branding
  ...
  - [x] 7.3 Add branded login route to App.tsx
...
- [x] 8. Frontend - Settings page restructure and slug management
  - [x] 8.1 Rename Master Data page to Settings and add Center tab
    - Rename `MasterDataPage.tsx` to `SettingsPage.tsx` (or refactor in place)
    - Update the page title from "Master Data" to "Settings" and subtitle accordingly
    - Add a new "Center" tab alongside existing "Batches" and "Drills" tabs
    - Keep the `/master-data` route working (or update to `/settings`) and update any nav references
    - Update the TopNav "Settings" label/path if the route changes
    - The "Center" tab is only visible to HEAD_COACH and ADMIN roles
    - _Requirements: 7.1, 7.6_

  - [x] 8.2 Add slug editor in the Center settings tab
    - Create a `CenterSettingsTab` component rendered inside the Center tab
    - Fetch the current center's slug from the API (or from auth context if available)
    - Display the current slug value pre-filled in an editable field
    - Show the branded login URL preview: `/login/{slug}`
    ...
    - Restrict editing to HEAD_COACH and ADMIN roles
